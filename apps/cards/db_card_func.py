...
async def db_get_card(session, card_table, card_name):
    exist_card = await session.scalar(select(card_table).where(card_table.card_name == card_name))
    return exist_card


async def db_get_all_card(session, card_table, user_id):
    card_massive = await session.scalars(select(card_table).where(card_table.user_id == user_id))
    return card_massive


async def db_add_card(session, card_table, card):
    created_card = session.add(card_table(user_id=card.user_id, card_name=f"{card.user_id}{card.card_name}", card_path=os.path.join(StoragePath, str(card.user_id)), group=card.group, favorites=card.favorites))
    exist_card = await session.scalar(select(card_table).where(card_table.card_name == card.card_name))
    return exist_card
