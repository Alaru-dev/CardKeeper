...
async def create_card_controller(
    ...
    card = CreateCardRequest(
        user_id=user_id, card_name=card_name, group=group, favorites=None
    )
    async with async_session() as session, session.begin():
        not_uniq_card_name = await db_get_card(
            session, Card, f"{card.user_id}{card.card_name}"
        )
        if not not_uniq_card_name:
            ...
            card_storage_absolute_path = os.path.join(
                user_storage_dir, add_new_file_name(image.filename, card_name)
            )
